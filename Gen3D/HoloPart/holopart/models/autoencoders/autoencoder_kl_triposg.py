...
class TripoSGVAEModel(ModelMixin, ConfigMixin):
    ...
    @apply_forward_hook
    def encode(
        self, x: torch.Tensor, num_tokens=2048, return_dict: bool = True, **kwargs
        ...
        if self.use_slicing and x.shape[0] > 1:
            encoded_slices = [
                self._encode(x_slice, num_tokens=num_tokens, **kwargs)
                for x_slice in x.split(self.slicing_length)
            ]
            h = torch.cat(encoded_slices)
        else:
            h = self._encode(x, num_tokens=num_tokens, **kwargs)
