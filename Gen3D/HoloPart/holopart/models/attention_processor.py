...
class FlashTripoSGAttnProcessor2_0:
    ...
    used in the TripoSGDiT model. It applies a s normalization layer and rotary embedding on query and key vector.
    """
    ...
    def __call__(
        ...
        # NOTE that triposg split heads first then split qkv or kv, like .view(..., attn.heads, 3, dim)
...
class FusedTripoSGAttnProcessor2_0:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "FusedSGAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0."
            )
    ...
    def __call__(
        ...
        # NOTE that triposg split heads first, then split qkv
        if encoder_hidden_states is None:
