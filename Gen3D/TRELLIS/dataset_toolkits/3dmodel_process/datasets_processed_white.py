...
def parse_arguments():
    ...
    parser.add_argument('--in_glb', type=str, default="/home/seulxia/project/PartGen/4dcloth/model/s10/scans/00_000001_uv/00_000001_uv.obj",
                        help='Input 3D model file path or directory (supports .obj, .glb, .gltf, .blend formats)')
    ...
    parser.add_argument('--batch', action='store_true', help='Batch process all 3D model files (.obj, .glb, .gltf, .blend) in subdirectories')
    ...
    parser.add_argument('--separate_method', type=str, default='none', 
                        choices=['none', 'loose', 'material', 'both', 'smart_loose'],
                        help='Method to separate mesh parts: none (no separation), loose (separate loose parts), material (separate by material), both (try both methods), smart_loose (intelligent connectivity analysis)')
    parser.add_argument('--allow_single_part', action='store_true', 
                        help='Allow processing models with only 1 part (skip the >1 constraint)')
    ...
    print(f"DEBUG: Separation method={args.separate_method}, allow_single_part={args.allow_single_part}")
    return args
...
def import_glb_and_separate_parts(in_glb_path, separate_method='none'):
    """
    导入GLB/OBJ/BLEND文件并分离部件
    
    Args:
        in_glb_path: 模型文件路径
        separate_method: 分离方法 ('none', 'loose', 'material', 'both')
    """
    # Clear scene
    ...
    if file_ext in ['.obj']:
        ...
        print(f"Importing BLEND file: {in_glb_path}")
        # For .blend files, we load it using bpy.ops.wm.open_mainfile
        # Note: this replaces the current scene
        bpy.ops.wm.open_mainfile(filepath=in_glb_path)
    else:
        raise ValueError(f"Unsupported file format: {file_ext}. Supported formats: .obj, .glb, .gltf, .blend")

    # 应用分离方法
    if separate_method == 'smart_loose':
        print(f"Applying separation method: SMART_LOOSE (intelligent connectivity analysis)")
        ...
        current_parts = [o for o in bpy.context.scene.objects if o.type == 'MESH']
        print(f"  After LOOSE separation: {len(current_parts)} parts")
        
        # 第二步：如果还是单部件或部件太少，尝试弱连接检测
        if len(current_parts) <= 1:
            print(f"  Model remains single part, trying bridge detection...")
            for obj in list(current_parts):
                if obj.type == 'MESH':
                    separate_by_weak_connectivity(obj, method='bridge_detection')
        
        # 第三步：如果bridge_detection效果不好，尝试简单分割
        current_parts = [o for o in bpy.context.scene.objects if o.type == 'MESH']
        if len(current_parts) <= 1:
            print(f"  Still single part, trying simple edge split...")
            for obj in list(current_parts):
                if obj.type == 'MESH':
                    separate_by_weak_connectivity(obj, method='simple_split')
        
        print(f"  Final parts count after SMART_LOOSE: {len([o for o in bpy.context.scene.objects if o.type == 'MESH'])}")
    elif separate_method in ['loose', 'both']:
        print(f"Applying separation method: LOOSE (separate disconnected geometry)")
        # 分离松散部件（没有连接的几何体）
        for obj in list(bpy.context.scene.objects):
            if obj.type != 'MESH':
                continue
            ...
            bpy.ops.object.mode_set(mode='OBJECT')
            obj.select_set(False)
        print(f"  After LOOSE separation: {len([o for o in bpy.context.scene.objects if o.type == 'MESH'])} parts")
    
    if separate_method in ['material', 'both']:
        print(f"Applying separation method: MATERIAL (separate by material)")
        # 按材质分离
        for obj in list(bpy.context.scene.objects):
            if obj.type != 'MESH':
                continue
            ...
            if len(obj.data.materials) > 1:
                ...
                bpy.ops.object.mode_set(mode='OBJECT')
                obj.select_set(False)
        print(f"  After MATERIAL separation: {len([o for o in bpy.context.scene.objects if o.type == 'MESH'])} parts")
    ...
    print(f"Final parts count: {len(mesh_objs)}")
...
def separate_by_weak_connectivity(obj, method='bridge_detection'):
    """
    检测弱连接区域并分离
    ...
    Args:
        obj: 要分离的mesh对象
        method: 检测方法
            - 'bridge_detection': 检测桥接边（连接大块区域的少量边）
            - 'simple_split': 简单的边界边分割
    """
    import bmesh
    
    print(f"  Analyzing weak connectivity for {obj.name}...")
    bpy.context.view_layer.objects.active = obj
    bpy.ops.object.mode_set(mode='OBJECT')
    obj.select_set(True)
    bpy.ops.object.mode_set(mode='EDIT')
    ...
    if method == 'bridge_detection':
        ...
        for edge in bm.edges:
            ...
            if len(edge.link_faces) <= 2:
                # 计算两个顶点的度数（连接的边数）
                v1_degree = len(edge.verts[0].link_edges)
                v2_degree = len(edge.verts[1].link_edges)
                
                # 如果顶点度数较低，可能是桥接
                if v1_degree <= 4 or v2_degree <= 4:
                    bridge_edges.append(edge)
        
        print(f"    Found {len(bridge_edges)} potential bridge edges")
        ...
        if bridge_edges:
            ...
            separated_count = bpy.ops.mesh.separate(type='LOOSE')
            
            print(f"    Separated into parts after bridge detection")
    ...
    final_parts = [o for o in bpy.context.scene.objects if o.type == 'MESH']
    print(f"    Final parts after weak connectivity analysis: {len(final_parts)}")
...
def calculate_and_filter_parts(mesh_objs, min_part_ratio, max_parts, out_dir, allow_single_part=False):
    """
    计算部件体积并根据比例和数量约束进行过滤
    
    Args:
        allow_single_part: 是否允许单部件模型
    """
    ...
    # 检查部件数量上限
    if len(filtered_mesh_objs) > max_parts:
        print(f"Skip asset: parts count {len(filtered_mesh_objs)} exceeds max_parts ({max_parts}).")
        try:
            os.makedirs(out_dir, exist_ok=True)
            with open(os.path.join(out_dir, "meta.json"), 'w') as f:
                json.dump({"skipped": True, "reason": "too_many_parts", "parts": len(filtered_mesh_objs), "max_parts": max_parts}, f, indent=2)
        except Exception as e:
            print("Failed to write skip meta:", e)
        return None
    
    # 检查部件数量下限（如果不允许单部件）
    if not allow_single_part and len(filtered_mesh_objs) <= 1:
        print(f"Skip asset: parts count {len(filtered_mesh_objs)} <= 1 (use --allow_single_part to process single-part models).")
        try:
            os.makedirs(out_dir, exist_ok=True)
            with open(os.path.join(out_dir, "meta.json"), 'w') as f:
                json.dump({"skipped": True, "reason": "single_part", "parts": len(filtered_mesh_objs)}, f, indent=2)
        except Exception as e:
            print("Failed to write skip meta:", e)
        return None
...
def setup_camera_and_rendering(args):
    ...
    # World settings (helps overall brightness)
    if not scene.world:
    ...
    # Keep original lighting setup - don't add extra lights
...
# ----------------------
def generate_incomplete_part_images(visibility_data, out_dir):
    """
        ...
        try:
            ...
            for part_idx in vis_data['part_indices']:
                ...
                try:
                    ...
                    # Invisible area: white with full opacity
                    output_array[visible_mask] = whole_array[visible_mask]
                    output_array[~visible_mask] = [255, 255, 255, 255]  # White with full opacity
                    ...
                    incomplete_part_files.append(output_filename)
...
def render_individual_parts(mesh_objs, all_mesh_objs, dirs, center_all, fixed_cam_params, 
                           cam, scene, camera_logs, out_dir, camera_headlight=None):
    """渲染各个部件"""
    i = 0
    for o in mesh_objs:
        ...
        try:
            ...
            for cam_idx, d in enumerate(dirs):
                ...
                # record image path
                rel_fn = os.path.relpath(fn, out_dir)
...
def render_segment_images(mesh_objs, all_mesh_objs, dirs, cam, scene, camera_logs,
                         out_dir, part_colors, camera_headlight=None):
    ...
    try:
        ...
        if all_corners:
            ...
            for d in dirs:
                ...
                # 保存segment路径到camera log
                if cam_idx < len(camera_logs):
                    camera_logs[cam_idx]["whole_segment"] = os.path.relpath(fn, out_dir)
    ...
    try:
        ...
        if all_corners:
            ...
            for d in dirs:
                ...
                # save overall paths into corresponding camera entry before complete_part
                camera_logs[cam_idx]["whole"] = os.path.relpath(fn, out_dir)
...
def extract_masks_from_segment(camera_logs, mesh_objs, part_colors, out_dir):
    ...
    for cam_idx, cam_log in enumerate(camera_logs):
        ...
        segment_path = os.path.join(out_dir, cam_log["whole_segment"])
...
def process_segmentation_masks(camera_logs, mesh_objs, part_colors, out_dir):
    """处理分割掩码生成"""
    visibility_data = {}
    incomplete_part_data = {}
    try:
        ...
        # 从segment图像提取masks
        visibility_data = extract_masks_from_segment(camera_logs, mesh_objs, part_colors, out_dir)
        
        print(f"\n为 {len(visibility_data)} 个相机视图生成了visibility masks")
        
        # 从masks和whole图像生成incomplete part图像
        if visibility_data:
            incomplete_part_data = generate_incomplete_part_images(visibility_data, out_dir)
...
def process_single_model(model_path, output_dir, args):
    ...
    try:
        ...
        # 3. 导入GLB文件并分离部件
        all_mesh_objs, mesh_objs, original_materials = import_glb_and_separate_parts(IN, args.separate_method)
        
        # 4. 计算体积并过滤部件
        mesh_objs = calculate_and_filter_parts(mesh_objs, args.min_part_ratio, args.max_parts, OUT_DIR, args.allow_single_part)
        
        # 如果模型被跳过（部件数量不符合要求），则返回
        if mesh_objs is None:
            print(f"⊘ Skipped: {model_path} (部件数量不符合要求)\n")
            return 'skipped'
        ...
        camera_logs = render_individual_parts(
            ...
            cam, scene, camera_logs, OUT_DIR, camera_headlight)
        ...
        camera_logs = render_segment_images(
            ...
            OUT_DIR, part_colors, camera_headlight)
...
def main():
    ...
    if args.batch and os.path.isdir(args.in_glb):
        ...
        # 查找所有子目录中的3D模型文件（.obj, .glb, .gltf, .blend）
        model_files = []
        supported_extensions = ('.obj', '.glb', '.gltf', '.blend')
        ...
        for ext in supported_extensions:
            count = len([f for f in model_files if f.endswith(ext)])
            if count > 0:
                format_counts[ext] = count
        
        print(f"找到 {len(model_files)} 个3D模型文件:")
        ...
        for idx, model_path in enumerate(model_files, 1):
            # 获取相对路径并保持目录结构
            rel_path = os.path.relpath(model_path, args.in_glb)
            # 移除扩展名，保留目录结构
            subdir_path = os.path.splitext(rel_path)[0]
            output_dir = os.path.join(args.out_dir, subdir_path)
            
            print(f"\n[{idx}/{len(model_files)}] 处理文件: {rel_path}")
            result = process_single_model(model_path, output_dir, args)
